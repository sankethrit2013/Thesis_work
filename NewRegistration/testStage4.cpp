	...
	for (size_t t = 0; t < 5; t++)
	{
		if (t == 1)
		{
			for (int j = 0; j < sizeof(MaximumNumberOfIterations) / sizeof(MaximumNumberOfIterations[0]); j++)
			{
					...
					sprintf_s(labelmapOutputFolder, " E:/Sanketh_Moudgalya_Directory/Nate/ImageVolume/20170612/FinalLM/labelmap_Stage4_Iter%d", MaximumNumberOfIterations[j]);

					char outputfolder[300];
					sprintf_s(outputfolder, " E:/Sanketh_Moudgalya_Directory/Nate/ImageVolume/20170612/scan1/Stage4_Registration_Iterations%d", MaximumNumberOfIterations[j]);
...
#pragma endregion
					...
					char parameter[50];
					sprintf_s(parameter, " %d", MaximumNumberOfIterations[j]);

					char command[8000] = "";
					strcat_s(command, "E:/Sanketh_Moudgalya_Directory/Nate/C++_codes/built_codes/NewRegistration/Debug/Stage4Registration.exe ");
