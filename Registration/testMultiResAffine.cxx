...
int main()
{
	double translationFactor[] = { 0.3 };
	int iterationsNumber[] = { 200 };
	int numberOfLevels[] = { 1, 2, 3 };
	for (int j = 0; j < sizeof(numberOfLevels) / sizeof(numberOfLevels[0]); j++)
	{
		for (int k = 0; k < sizeof(iterationsNumber) / sizeof(iterationsNumber[0]); k++)
		{
			for (int i = 0; i < sizeof(translationFactor) / sizeof(translationFactor[0]); i++)
			{
				...
				char outputfolder[300];
				sprintf_s(outputfolder, " C:/Users/zx4403/Dropbox/Research/ITK/ImageVolume/20160108/MultiResAffine_first/Level%d_TransScale%g_IterationNum%d_", numberOfLevels[j], translationFactor[i], iterationsNumber[k]);
				...
				char parameter[50];
				sprintf_s(parameter, " %d %g %d", iterationsNumber[k], translationFactor[i], numberOfLevels[j]);

				char command[3000] = "D:/ITK4.8/ITK_Projects_build/Registration/Debug/MultiResAffine3D.exe ";
